...
class meuG : public Graph{
    public:
    //construtor so invoca da classe base
        meuG(bool directed): Graph(directed){}

    void leiaG(){
        std::string operacao, vertice1,vertice2;
        double peso;
        ...
        while(operacao != "f"){
            ...
            std::cin >> vertice1 >> vertice2 >> peso;
            this->addEdge(vertice1, vertice2 , peso);
            }
            else std::cout << "operação invalida!" << std::endl;
            std::cin >> operacao;
            }
...
int main(){
    ...
    g.exibeCaminhoMinimo(origem, destino);
